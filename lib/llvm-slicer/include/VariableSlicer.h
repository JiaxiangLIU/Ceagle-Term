...
namespace llvm { namespace slicing {
  ...
  class VariableSlicer {
    public:
      ...
      VariableSlicer(ModulePass *MP, Module &M,
...
namespace llvm { namespace slicing {
  template<typename OutIterator>
    void VariableSlicer::emitToForwardCalls(llvm::Function const* const f,
        OutIterator out) {
#ifdef DEBUG_EMIT
  ...
  template<typename OutIterator>
    void VariableSlicer::emitToCalls(llvm::Function const* const f,
        OutIterator out) {
#ifdef DEBUG_EMIT
      errs() << __func__ << " for " << f->getName() << "\n";
#endif
  ...
  template<typename OutIterator>
    void VariableSlicer::emitToForwardExits(llvm::Function const* const f,
        OutIterator out) {
#ifdef DEBUG_EMIT
      errs() << __func__ << " for " << f->getName() << "\n";
#endif
  ...
  template<typename OutIterator>
    void VariableSlicer::emitToExits(llvm::Function const* const f,
        OutIterator out) {
      // Pseudo-code:
      //
