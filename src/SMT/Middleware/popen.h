#ifndef CEAGLE_SMT_POPEN_H
#define CEAGLE_SMT_POPEN_H
...
public:
    ...
    Popen(const Popen& other) : pid(other.pid), infp(other.infp), outfp(other.outfp), debugInfo(other.debugInfo.str()) {
    }
    ~Popen() {
        close(infp);
        close(outfp);
    ...
    std::string getDebugInfo() const { return debugInfo.str(); }
private:
    bool isEnd(std::string str) const;
    static pid_t popen2(int* intfp, int* outfp, const char* file, char* const args[]);
};
